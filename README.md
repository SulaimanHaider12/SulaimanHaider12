# Hi, I'm Muhammad Sulaiman Haider 👋

### Odoo Technofunctional Developer | ERP Customization | Integrations

<p align="left">
<img src="https://upload.wikimedia.org/wikipedia/commons/5/50/Odoo_logo.svg" width="120" alt="Odoo"/>
</p>

I'm an **Odoo Technical Developer** focused on building, customizing, and improving ERP solutions that align with real business workflows.

I work across **Odoo development, custom modules, integrations, reporting, workflows, QA/UAT, and technical implementation**, while collaborating with clients and development teams in remote environments.

---

## 🚀 What I Do

* ⚙️ Odoo Customization & Development
* 🧩 Custom Modules & Business Workflows
* 🔗 Third-Party API & System Integrations
* 📊 Reports, Dashboards & QWeb
* 🏭 Manufacturing & Inventory Solutions
* 💼 Sales & CRM Workflows
* 🧪 QA & UAT Testing
* 🎨 Odoo Studio Customization
* 🤝 Requirement Gathering & Client Collaboration
* 📋 Task & Project Coordination with Asana

---

## 🛠️ Tech Stack

<p align="left">

<a href="https://www.odoo.com/" target="_blank">
<img src="https://cdn.simpleicons.org/odoo/714B67" width="45" alt="Odoo"/>
</a>

<a href="https://www.python.org/" target="_blank">
<img src="https://cdn.simpleicons.org/python/3776AB" width="45" alt="Python"/>
</a>

<a href="https://www.postgresql.org/" target="_blank">
<img src="https://cdn.simpleicons.org/postgresql/4169E1" width="45" alt="PostgreSQL"/>
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript" target="_blank">
<img src="https://cdn.simpleicons.org/javascript/F7DF1E" width="45" alt="JavaScript"/>
</a>

<a href="https://www.w3.org/XML/" target="_blank">
<img src="https://cdn.simpleicons.org/xml/005FAD" width="45" alt="XML"/>
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/HTML" target="_blank">
<img src="https://cdn.simpleicons.org/html5/E34F26" width="45" alt="HTML5"/>
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/CSS" target="_blank">
<img src="https://cdn.simpleicons.org/css/1572B6" width="45" alt="CSS3"/>
</a>

<a href="https://git-scm.com/" target="_blank">
<img src="https://cdn.simpleicons.org/git/F05032" width="45" alt="Git"/>
</a>

<a href="https://github.com/" target="_blank">
<img src="https://cdn.simpleicons.org/github/181717" width="45" alt="GitHub"/>
</a>

<a href="https://www.docker.com/" target="_blank">
<img src="https://cdn.simpleicons.org/docker/2496ED" width="45" alt="Docker"/>
</a>

<a href="https://www.postman.com/" target="_blank">
<img src="https://cdn.simpleicons.org/postman/FF6C37" width="45" alt="Postman"/>
</a>

</p>

**Odoo:**
Odoo 12–19 · Odoo.sh · Odoo Studio · Sales · CRM · Inventory · Manufacturing · Reporting

**Development:**
Python · JavaScript · XML · QWeb · HTML · CSS · OWL · REST API · JSON-RPC

**Database & DevOps:**
PostgreSQL · Git · GitHub · Docker · Linux / Ubuntu

**Tools:**
PyCharm · Postman · Asana

---

## 📌 Selected Projects

> **🔒 Client Confidentiality Notice**
>
> Due to **NDA and client confidentiality agreements**, the source code, screenshots, repositories, and complete project details of some client projects cannot be publicly displayed.
>
> However, I can **walk through the projects, explain the technical architecture, workflows, customizations, integrations, and my contributions** during a **Google Meet or Zoom call**, where appropriate and permitted.

### Mega Wholesale LLC

**Odoo Implementation · Bidding & Sales Workflow**

Implemented a bidding-to-sales workflow with automated lead routing based on won and lost outcomes.

### Tailorbird Ventures

**Odoo Implementation · Bidding & Sales Workflow**

Customized the bidding-to-sales process according to the client's business workflow.

### Veteran Legacy

**Odoo Implementation · Bidding & Sales Workflow**

Built a lead-to-bid pipeline with automated workflow progression based on bid outcomes.

### GET Technologies Group

**Odoo Database Architecture · Custom Integrations**

Designed the Odoo database structure for a manufacturing business and developed custom integrations with third-party systems.

### Core2Plus (C2P Consultants)

**Odoo Services · Implementation & Integrations**

Worked on Odoo implementations, customizations, and integrations across multiple client projects.

---

## 🌍 Working Style

* Remote & Cross-Time-Zone Collaboration
* Requirement Analysis
* Structured Problem Solving
* Client-Focused Implementation
* QA & UAT Before Delivery
* Continuous Learning

---

## 🤝 Connect With Me

<p align="left">
<a href="https://www.linkedin.com/in/sulaiman-haider-web-developer/" target="_blank">
<img src="https://img.shields.io/badge/LinkedIn-Muhammad%20Sulaiman%20Haider-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
</p>

📧 **Email:** [sulaimanhaider.sabir@gmail.com](mailto:sulaimanhaider.sabir@gmail.com)
📱 **WhatsApp:** +92 307 1201650

---

⭐ *Building practical Odoo solutions that turn business requirements into working systems.*
